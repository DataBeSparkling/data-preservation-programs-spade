...
LANGUAGE sql PARALLEL SAFE IMMUTABLE STRICT AS $$
  ...
  -- weird magic numbers due to chain starting at 22:00 UTC Monday
  SELECT ( ( ( $1 + 2640 ) / 20160 )::INTEGER * 20160 - 2640 )
$$;
...
CREATE TABLE IF NOT EXISTS spd.datasets (
  ...
  dataset_slug TEXT NOT NULL CONSTRAINT dataset_slug_lc CHECK ( dataset_slug ~ '^[a-z0-9\-\_]+$' ),
  dataset_meta JSONB NOT NULL DEFAULT '{}',
  CONSTRAINT tenant_dataset_ref UNIQUE ( tenant_id, dataset_id ), -- pieces_datasets, datasets_links FK
  CONSTRAINT tenant_dataset_slug UNIQUE ( dataset_slug, tenant_id ) -- pieces_datasets, datasets_links FK
);

CREATE TABLE IF NOT EXISTS spd.datasets_links (
  superset_id INTEGER NOT NULL,
  subset_id INTEGER NOT NULL,
  tenant_id SMALLINT NOT NULL REFERENCES spd.tenants ( tenant_id ) ON UPDATE CASCADE,
  CONSTRAINT datasets_links_singleton UNIQUE ( superset_id, subset_id ),
  CONSTRAINT datasets_linls_super_fk FOREIGN KEY ( superset_id, tenant_id ) REFERENCES spd.datasets ( dataset_id, tenant_id ) ON UPDATE CASCADE,
  CONSTRAINT datasets_linls_sub_fk FOREIGN KEY ( subset_id, tenant_id ) REFERENCES spd.datasets ( dataset_id, tenant_id ) ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS spd.labels (
  label_id BIGSERIAL NOT NULL,
  label_string BYTEA NOT NULL
);
CREATE UNIQUE INDEX IF NOT EXISTS labels_id_key ON spd.labels ( label_id ) INCLUDE ( label_string );
CREATE UNIQUE INDEX IF NOT EXISTS labels_string_key ON spd.labels ( label_string ) INCLUDE ( label_id );
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 42
      FROM information_schema.table_constraints
    WHERE
      constraint_schema = 'spd'
        AND
      table_name = 'labels'
        AND
      constraint_name = 'labels_id_key'
  ) THEN
    ALTER TABLE spd.labels ADD CONSTRAINT labels_id_key UNIQUE USING INDEX labels_id_key;
    ALTER TABLE spd.labels ADD CONSTRAINT labels_string_key UNIQUE USING INDEX labels_string_key;
  END IF;
END $$;

CREATE TABLE IF NOT EXISTS spd.tenants_pieces (
  piece_id BIGINT NOT NULL REFERENCES spd.pieces ( piece_id ) ON UPDATE CASCADE,
  label_id BIGINT REFERENCES spd.labels ( label_id ),
  entry_created TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CLOCK_TIMESTAMP(),
  dataset_id INTEGER NOT NULL,
  tenant_id SMALLINT NOT NULL,
  piece_inactivated BOOL NOT NULL DEFAULT false,
  tenant_piece_meta JSONB NOT NULL DEFAULT '{}',
  CONSTRAINT tenants_pieces_singleton UNIQUE ( piece_id, dataset_id ),
  CONSTRAINT tenants_pieces_datasets_fk FOREIGN KEY ( tenant_id, dataset_id ) REFERENCES spd.datasets ( tenant_id,  dataset_id ) ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS spd.tenants_providers (
  provider_id INTEGER NOT NULL REFERENCES spd.providers ( provider_id ),
  tenant_id SMALLINT NOT NULL REFERENCES spd.tenants ( tenant_id ) ON UPDATE CASCADE,
  provider_inactivated BOOL NOT NULL DEFAULT false,
      ...
      FROM (
        (
          SELECT
            ...
            JOIN spd.tenants_pieces tp USING ( piece_id )
            LEFT JOIN spd.invalidated_deals id USING ( deal_id )
          WHERE
            NOT tp.piece_inactivated
              AND
            id.deal_id IS NULL
              AND
  ...
  SELECT
      ROW_NUMBER() OVER(
        ...
        ORDER BY
          ...
          uri_available DESC,
          ( coarse_latest_active_end_epoch IS NOT NULL ),
          recent_ts DESC NULLS LAST,
          coarse_latest_active_end_epoch NULLS FIRST,
          piece_cid
      ) AS display_sort,
      *,
      ( uri_available OR coarse_latest_active_end_epoch IS NOT NULL ) AS has_sources
    FROM (
      WITH
        pieces_of_interest AS (
          SELECT
              tp.piece_id,
              MAX( tp.entry_created ) AS recent_ts,
              ARRAY_AGG( DISTINCT( tp.tenant_id ) ) AS potential_tenant_ids
            FROM spd.tenants_pieces tp
          WHERE NOT tp.piece_inactivated
          GROUP BY tp.piece_id
        )
      SELECT
          p.piece_id,
          (
            SELECT
                spd.coarse_epoch( MAX( kdr.end_epoch ) )
              FROM spd.known_deals_ranked kdr
              WHERE kdr.piece_id = p.piece_id
          ) AS coarse_latest_active_end_epoch,
          p.piece_log2_size,
          ( p.piece_log2_size = 36 ) AS requires_64g_sector,
          true AS uri_available,
          p.piece_cid,
  ...
  WITH
    ...
    available_tenants AS (
      SELECT
          ...
          COALESCE( (tpr.tenant_provider_meta->'max_in_flight_GiB')::BIGINT,  (t.tenant_meta->'max'->'default_in_flight_GiB')::BIGINT, 1024 )::BIGINT << 30 AS max_in_flight_bytes,
          ...
          t.tenant_meta
        FROM ctx
        JOIN spd.tenants_providers tpr
          ON tpr.provider_id = ctx.provider_id AND NOT tpr.provider_inactivated
        JOIN spd.tenants t USING ( tenant_id )
        JOIN spd.tenants_pieces tpc
          ON tpc.piece_id = ctx.piece_id AND tpc.tenant_id = tpr.tenant_id AND NOT tpc.piece_inactivated
        LEFT JOIN tenant_addresses ta
          ON ta.tenant_id = tpr.tenant_id
    ),
    eligibility AS MATERIALIZED (
      SELECT
          ...
          COALESCE (
            ( SELECT
                  l.label_string
                FROM spd.tenants_pieces tp, spd.labels l
              WHERE
                tp.label_id = l.label_id
                  AND
                tp.piece_id = ctx.piece_id
                  AND
                tp.tenant_id = at.tenant_id
            ),
            CONVERT_TO( arg_piece_cid, 'UTF8' )
          ) AS proposal_label,
  ...
  (
    (
      ...
      GROUP BY GROUPING SETS (
        @{[ ( $_ eq q{continent} ) ? q{( piece_id, claimant_id ),} : q{} ]}
