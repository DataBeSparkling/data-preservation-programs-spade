...
func getSPInfo(ctx context.Context, sp filaddr.Address, timeOut time.Duration) (spInfo, error) {
	...
	mi, err := gctx.LotusAPI.StateMinerInfo(ctx, sp, fil.LotusTSK{})
	if err != nil {
	...
	spi.AddrInfo, err = lp2p.AssembleAddrInfo(mi.PeerId, mi.Multiaddrs)
	if err != nil {
		spi.Errors = append(spi.Errors, err.Error())
	}

	if spi.PeerID == nil || len(spi.MultiAddrs) == 0 {
		spi.Errors = append(spi.Errors, "SP is not dialable given on-chain info")
		return spi, nil
	}
	...
	unprot, dtm, err := lp2p.ConnectAndProtect(ctx, nodeHost, spi.AddrInfo)
