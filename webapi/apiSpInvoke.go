...
func apiSpInvoke(c echo.Context) (defErr error) {
	...
	return ctxMeta.Db[app.DbMain].BeginFunc(ctx, func(tx pgx.Tx) error {
		...
		if ctxMeta.signedArgs.Get("tenant_policy") != app.TEMPPolicies[chosenTenant.TenantID] {
			return retFail(
				c,
				apitypes.ErrInvalidRequest,
		...
		gbpce, err := providerCollateralEstimateGiB(
			...
			((startEpoch-app.FilDefaultLookback-(filbuiltin.EpochsInHour*filabi.ChainEpoch(chosenTenant.StartWithinHours))-240)/2880)*2880+240,
		)
		if err != nil {
			return cmn.WrErr(err)
		}
		...
		if len(chosenTenant.ProposalLabel) > 0 {
			// encodedLabel, err := filmarket.NewLabelFromBytes(chosenTenant.ProposalLabel)
			encodedLabel, err := filmarket.NewLabelFromString(string(chosenTenant.ProposalLabel))
			if err != nil {
				return cmn.WrErr(err)
			}

			// Label is a *completely* arbitrary, tenant-chosen nonce to apply to the deal
			// Use exclusively bytes instead of utf8
			prop.ProposalV0.Label = encodedLabel
		}
...
func providerCollateralEstimateGiB(ctx context.Context, sourceEpoch filabi.ChainEpoch) (filbig.Int, error) { //nolint:revive
	...
	if err != nil {
		tsj, _ := ts.Key().MarshalJSON()
		return filbig.Zero(), xerrors.Errorf("unable to determine provider collateral bounds for tipset %d %s: %w", sourceEpoch, tsj, err)
	}
